...
export const Checkbox: React.FC<Props> = ({ label, value, onChange }) => {
  ...
  return (
    <label className="inline-flex items-center cursor-pointer space-x-3">
      <input
        type="checkbox"
        className="w-6 h-6 border shadow-inner outline-none appearance-none cursor-pointer form-tick rounded-md checked:bg-primary-400 checked:border-transparent transition-all duration-300"
        checked={value}
